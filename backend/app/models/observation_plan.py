...
class ObservationPlan(Base, UUIDPrimaryKeyMixin, TimestampMixin, VersionedMixin, ProvenanceMixin):
    __tablename__ = "observation_plans"
    __table_args__ = (UniqueConstraint("project_id", name="uq_observation_plans_project_id"),)
    ...
    selected_duration: Mapped[float | None] = mapped_column(Float, nullable=True)
    unit: Mapped[str] = mapped_column(String(16), nullable=False, default="h")
    final_sampling_time: Mapped[float | None] = mapped_column(Float, nullable=True)
