...
### 2.4.1. Чтение таблиц PK (фаза 29.2)

Вариабельность в источниках почти всегда стоит в таблице, а не во фразе: строка называет
PK-параметр, столбец — статистику. Регулярки по фразам такие числа не видели, поэтому CVintra
приходилось вводить руками даже когда источник её заявляет.

`research_tables.py` восстанавливает строки таблицы из извлечённого текста и сообщает значение
только тогда, когда и строка, и столбец говорят, что это за число:

- строки собираются из текста PDF (столбцы разделены двумя+ пробелами), DOCX (`|` из ingest
  фазы 14.1) и HTML;
- HTML-сетка больше не теряется: `html_text_with_tables()` ставит границы ячеек и строк до
  снятия тегов (`research_fetch` использует её вместо `strip_html`), иначе столбец CV сливался
  с соседними и терял смысл;
- поддержаны обе ориентации: PK-параметр по строкам со столбцом CV и транспонированная таблица
  (PK-параметры в заголовке, строка «Intra-subject CV (%)»);
- формат `Geometric Mean (CV%)` читается правильно: CV берётся из скобок, а не среднее;
- тип вариабельности читается из подписи столбца, затем из заголовка таблицы, затем из текста
  блока — первым определённым; не нашли → `UNKNOWN`, догадки нет;
- `NR`, `NA`, `—`, пустая ячейка — значения не заявлено, предложения нет;
- диапазон в ячейке («20-35») остаётся диапазоном;
- **строка, не совпадающая с заголовком по числу ячеек, не читается вовсе** — пропавшая пустая
  ячейка сдвигает столбцы, и значение ушло бы под чужую подпись. Единственное исключение —
  сдвиг ровно на одну ячейку, когда в заголовке пуст левый верхний угол;
- число вне диапазона 0–200% не принимается за CV (год, доза, N).

Выдержка предложения показывает, что именно проверять: подпись таблицы, строку, столбец и строку
целиком — `Таблица «Table 3. …» · строка «Cmax (ng/mL)» · столбец «Intra-subject CV (%)» → 24.5`.

Значение из ячейки не разбирается заново: `TableFinding.as_metadata()` передаёт параметр, число
и тип вариабельности в ту же `extract_claims_from_hit`, поэтому у табличного и текстового путей
один набор инвариантов. Дедупликация предложений учитывает PK-параметр: одна таблица может
заявить одинаковый CV для Cmax и AUC, и оба предложения должны сохраниться.
...
### 2.8. Введённое значение больше не теряется и не возвращает в круг

Живой прогон упирался в петлю: писатель вводил Tmax / t½ / CVintra, после обновления экрана
пробел открывался снова, «Решения» слали обратно в «Пробелы», до генерации DOCX дойти
было нельзя. Причины, закрытые сейчас:

- `evaluate_cvintra_eligibility` переписывала `usability` под домен `STATISTICS`. Claim из
  поиска помечен `decision_domains=["DESIGN"]` — после чтения панели Sample Size он становился
  `NOT_USABLE`, статус пробела откатывался, значение выглядело «не записанным».
- Persist research-bag смотрел только на `tasks`. Экспертный ввод без новой задачи (или с
  claims при пустых tasks) затирался старым payload после `after_mutation`.
- `GET` ведомости после `fill` строился из пустой памяти: persist чистил кэш и не гидратировал
  его обратно.
- Подтверждённый пробел оставался в открытом списке (`needs_apply`), и next-action снова
  вела на ввод.

Теперь проверка eligibility только читает claim; CVintra из поиска несёт и DESIGN, и
STATISTICS; ручной ввод пишет полный `cvintra` и оба домена; bag не выбрасывает claims;
подтверждённое значение уходит в `resolved` и не спрашивается повторно. Следующий шаг —
расчёт N / выбор PRIMARY BE, а не повторный ввод.
...
| `app/domain/research_deep_read.py` | новый — чтение полного текста источников, вырезка фрагментов, чтение таблиц |
| `app/domain/research_tables.py` | новый (29.2) — восстановление строк таблицы, столбец вариабельности, сохранение HTML-сетки |
| `app/domain/research_fetch.py` | HTML читается с сохранением сетки таблиц |
...
`tests/test_research_deep_read.py` (10), `tests/test_research_tables.py` (17) — итого 49.
...
- Таблицы читаются (фаза 29.2), но только когда извлечённый текст сохранил границы столбцов.
  `pypdf` иногда отдаёт строку таблицы через один пробел — такая строка не распознаётся как
  сетка и остаётся непрочитанной. Следующий шаг — разбор по координатам символов (`pdfplumber`)
  вместо разделения по пробелам.
- Таблица, разорванная переносом страницы (заголовок на одной странице, строки на следующей),
  читается как блок без подписи: тип вариабельности тогда берётся из текста блока или остаётся
  `UNKNOWN`.
